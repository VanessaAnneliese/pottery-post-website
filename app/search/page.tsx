...
function highlight(text: string, query: string) {
  ...
  return parts.map((part, i) =>
    regex.test(part)
      ? <mark key={i} style={{ background: "#D4622A", color: "#F5F0E8", borderRadius: "2px", padding: "0 2px" }}>{part}</mark>
      : part
  );
}
...
function GuildResult({ guild, query }: { guild: Guild; query: string }) {
  return (
    <div className="py-5 border-b" style={{ borderColor: "#E8D5B7" }}>
      ...
      {guild.website && (
        <a href={guild.website} target="_blank" rel="noopener noreferrer" className="inline-block mt-2 text-xs tracking-widest uppercase underline" style={{ color: "#C1440E", fontFamily: "system-ui, sans-serif" }}>
          Website
        </a>
      )}
    </div>
...
function PotterResult({ potter, query }: { potter: Potter; query: string }) {
  return (
    <div className="py-5 border-b" style={{ borderColor: "#E8D5B7" }}>
      <p className="text-xs tracking-widest uppercase mb-1" style={{ color: "#9E8572", fontFamily: "system-ui, sans-serif" }}>Potter</p>
      ...
      {potter.website && (
        <a href={potter.website} target="_blank" rel="noopener noreferrer" className="inline-block mt-2 text-xs tracking-widest uppercase underline" style={{ color: "#C1440E", fontFamily: "system-ui, sans-serif" }}>
          Website
        </a>
      )}
    </div>
...
export default async function SearchPage({
  ...
  return (
    <section className="py-20 px-6 max-w-4xl mx-auto">
      ...
      {!query && (
        <p className="mt-6 max-w-lg" style={{ color: "#9E8572", fontFamily: "system-ui, sans-serif" }}>
      ...
      {matchedGuilds.length > 0 && (
        <div className="mt-8">
          <h2 className="text-xl font-bold mb-1" style={{ fontFamily: "Georgia, serif", color: "#D4622A" }}>Guilds</h2>
      ...
      {matchedPotters.length > 0 && (
        <div className="mt-10">
          <h2 className="text-xl font-bold mb-1" style={{ fontFamily: "Georgia, serif", color: "#D4622A" }}>Potters</h2>
      ...
      {query && total === 0 && (
        <div className="mt-10">
          <p className="mb-6" style={{ color: "#5C3D2E", fontFamily: "system-ui, sans-serif" }}>
