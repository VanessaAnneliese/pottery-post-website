...
export default function QuoteBlock({ quote, className }: { quote: ReactNode; className?: string }) {
  return (
    <section className={`relative px-6 text-center flex flex-col items-center justify-center min-h-[400px] ${className ?? ""}`} style={{ background: "#EDE4D5" }}>
      ...
      <div className="absolute inset-y-0 left-0 w-24 md:w-48 pointer-events-none" style={{ background: "linear-gradient(to right, #F5F0E8, transparent)", zIndex: 0 }} />
      <div className="absolute inset-y-0 right-0 w-24 md:w-48 pointer-events-none" style={{ background: "linear-gradient(to left, #F5F0E8, transparent)", zIndex: 0 }} />
